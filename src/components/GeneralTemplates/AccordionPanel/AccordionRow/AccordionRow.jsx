...
const AccordionRow = ({content, key, isExpandedAll, setIsExpandedAll}) => {

    console.log(content.googleMaps)
  ...
  return (
    <div className='accordionRow'>
        <button className={`accordionRow__button ${isRowClicked || isExpandedAll ? 'accordionRow__button--open' : ''}`} type='button' onClick={handleRowClick}>
            <div>
                <h2>{content.buildingName}</h2>
                <p>{`${totalAvailableStations} computers available`}</p>
            </div>
            <img src={arrowDownBlue} alt='arrow icon' className={`accordionRow__button-icon ${isRowClicked || isExpandedAll ? 'accordionRow__button-icon--rotated' : ''}`}/>
        </button>
